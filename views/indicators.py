...
def calculate_indicators(data):
    indicators_df = pd.DataFrame({
        'Name': data['Name'],
        'Win Rate': data['Wins'] / (data['Wins'] + data['Losses']),
        'Physical Condition': (data['Height']/data['Height'].max() + 
                             data['Weight']/data['Weight'].max() + 
                             pd.factorize(data['Health Status'])[0]/pd.factorize(data['Health Status'])[0].max()) / 3,
        'Mental Resilience': pd.factorize(data['Mental Resilience'])[0] / 2,  # Normalizado a 0-1
        'Public Popularity': data['Public Favor'],
        'Battle Experience': data['Battle Experience']
    })
...
def show():
    ...
    if 'data' in st.session_state:
        ...
        st.subheader("Valores de los Indicadores por Gladiador")
        st.dataframe(indicators_df.style.format({
            'Win Rate': '{:.2%}',
            'Physical Condition': '{:.2f}',
            'Mental Resilience': '{:.2f}',
            'Public Popularity': '{:.2f}'
        }))
        ...
        st.write("Primero deberá ingresar el documento de gladiator, la lectura de otros documentos se encuentra pendiente.")